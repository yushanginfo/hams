...
import net.yushanginfo.hams.account.model.{Bankcard, BankcardStat}
...
class BankcardStatAction extends ActionSupport, EntityAction[Bankcard] {
  ...
  def stat(): View = {
    ...
    if (stats.isEmpty || forceStat) {
      stats = bankcardService.stat(ym, forceStat)
    }
    put("stats", stats)
