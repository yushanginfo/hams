...
[@b.grid items=bills var="bill"]
  [@b.gridbar]
    ...
    var m=bar.addMenu("${b.text("action.export")}",action.exportData("wallet.inpatient.code:住院号,wallet.inpatient.name:姓名,"+
                 ...
                 "amount:支出金额,goods:消费明细,payAt:支出时间",null,'fileName=伙食费支出流水'));
  ...
  [@b.row]
    ...
    [@b.col property="wallet.inpatient.name" title="姓名"]
      [@b.a href="!info?id=${bill.id}"]
        <span title="${bill.wallet.inpatient.beginAt?string('yyyy-MM-dd')}~${(bill.wallet.inpatient.endAt?string('yyyy-MM-dd'))!}">${bill.wallet.inpatient.name}</span>
      [/@]
    [/@]
    ...
    [@b.col width="13%" property="goods" title="消费明细"/]
    [@b.col width="13%" property="payAt" title="入账时间"]
      ${bill.payAt?string('yy-MM-dd HH:mm')}
    [/@]
  [/@]
[/@]
[@b.foot/]
