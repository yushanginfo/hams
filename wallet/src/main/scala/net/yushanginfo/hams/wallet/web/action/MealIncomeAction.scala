...
class MealIncomeAction extends RestfulAction[Income], ImportSupport[Income], ExportSupport[Income] {
  ...
  override protected def saveAndRedirect(income: Income): View = {
    ...
    if (!income.persisted && !Strings.isEmpty(income.wallet.inpatient.code)) {
      inpatientService.getInpatient(income.wallet.inpatient.code) match {
        ...
        case Some(inpatient) =>
          val wallet = walletService.getOrCreateWallet(inpatient, WalletType.Meal, income.payAt)
  ...
  def downloadTemplate(): View = {
    ...
    schema.generate(os)
    Stream(new ByteArrayInputStream(os.toByteArray), MediaTypes.ApplicationXlsx.toString, "伙食费入账.xlsx")
  }

  protected override def configImport(setting: ImportSetting): Unit = {
    setting.listeners = List(new ForeignerListener(entityDao),
      new IncomeImportListener(WalletType.Meal, inpatientService, walletService, entityDao))
  }
}
