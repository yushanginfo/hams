...
class PensionIncomeAction extends RestfulAction[Income], ImportSupport[Income], ExportSupport[Income] {
    ...
    if (!income.persisted && !Strings.isEmpty(income.wallet.inpatient.code)) {
      entityDao.findBy(classOf[Inpatient], "code", income.wallet.inpatient.code).headOption match {
        ...
        case Some(i) =>
          ...
          q.where("wallet.inpatient=:inpatient and wallet.walletType=:type", i, WalletType.Pension)
          val wallet = entityDao.search(q).headOption match {
            case Some(w) => w
            case None =>
              val w = Wallet(i, WalletType.Pension)
  ...
  override protected def getQueryBuilder: OqlBuilder[Income] = {
    ...
    query.where("income.wallet.walletType=:walletType", WalletType.Pension)
    query
  }
}
