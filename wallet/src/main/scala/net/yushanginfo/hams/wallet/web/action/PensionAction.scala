...
class PensionAction extends RestfulAction[Wallet], ImportSupport[Wallet], ExportSupport[Wallet] {
  ...
  override protected def saveAndRedirect(wallet: Wallet): View = {
    if (!wallet.persisted && !Strings.isEmpty(wallet.inpatient.code)) {
      ...
      wallet.walletType = WalletType.Pension
  ...
  override protected def getQueryBuilder: OqlBuilder[Wallet] = {
    ...
    query.where("wallet.walletType=:walletType", WalletType.Pension)
    query
  }
