 ...
 * 伙食费支出
 */
class PensionBillAction extends RestfulAction[Bill], ImportSupport[Bill], ExportSupport[Bill] {
  ...
  override protected def getQueryBuilder: OqlBuilder[Bill] = {
    ...
    query.where("bill.wallet.walletType=:walletType", WalletType.Pension)
    query
  }
}
